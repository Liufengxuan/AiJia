...
func init(){
	//注册数据库
	orm.RegisterDataBase("default","mysql","root:8139@tcp(127.0.0.1:3306)/aijiadb?charset=utf8",30)
	//创建表
	orm.RegisterModel(new(User),new(House),new(Area),new(Facility),new(HouseImage),new(OrderHouse))
	//force:true如果存在就更新 false:如果表存在就不创建了，verbose：不存在就创建，false：不存在就不执行
